...
function validateUsername() {
    ...
    if ($username.length < 6) {
        $('#username-status').html('<span class="warning">Username must be 6 symbols or more!</span>');
        $('#username').addClass('invalid');
    } else {
        $('#username-status').html('<span class="valid">Valid Username!</span>');
...
function validatePassword() {
    ...
    if ($pass1 !== $pass2) {
        $('#confirm-status').html('<span class="warning">Passwords don\'t match!</span>');
        $('#confirm').addClass('invalid');
    } else {
        $('#confirm-status').html('<span class="valid">Passwords match!</span>');
